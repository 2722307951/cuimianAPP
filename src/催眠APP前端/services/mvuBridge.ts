...
export const MvuBridge = {
  ...
  syncUserResources: async (user: UserResources) => {
    return enqueueMvuWrite(async () => {
      ...
      if (await setIfChanged(mvu, '系统._催眠能量', user.mcEnergy)) changed = true;
      if (await setIfChanged(mvu, '系统._催眠能量上限', user.mcEnergyMax)) changed = true;
      if (await setIfChanged(mvu, '主角.金钱', user.money)) changed = true;
      if (await setIfChanged(mvu, '系统.可疑度', user.suspicion)) changed = true;
  ...
  setProtagonistItem: async (itemName: string, description: string) => {
    return enqueueMvuWrite(async () => {
      const data = await getMvuData();
      if (!data) return false;
      const { mvu, option } = data;

      const path = `主角.持有物品.${itemName}`;
      const prev = _.get(mvu.stat_data, path);
      if (_.isEqual(prev, description)) return false;

      _.set(mvu.stat_data, path, description);
      await Mvu.replaceMvuData(mvu, option);
      return true;
    });
  },

  getProtagonistItems: async (): Promise<Record<string, string>> => {
    const data = await getMvuData();
    if (!data) return {};
    const items = _.get(data.mvu.stat_data, '主角.持有物品');
    return _.isPlainObject(items) ? (items as Record<string, string>) : {};
  },
  ...
  syncSubscriptionState: async (store: unknown, tierValue: number) => {
    return enqueueMvuWrite(async () => {
      const data = await getMvuData();
      if (!data) return;
      ...
      if (await setIfChanged(mvu, '系统._hypnoos', store)) changed = true;
      if (await setIfChanged(mvu, '系统._催眠APP等级', tierValue)) changed = true;
  ...
  syncSubscriptionTier: async (tierValue: number) => {
    return enqueueMvuWrite(async () => {
      ...
      const { mvu, option } = data;
      const changed = await setIfChanged(mvu, '系统._催眠APP等级', tierValue);
      if (changed) {
        await Mvu.replaceMvuData(mvu, option);
      }
    });
