...
const BAR_STATS = new Set(['警戒度', '色情度', '好感度', '性欲', '快感值']);
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-white/60 mt-1 leading-relaxed">
                  该模块属于 LV1 默认能力，可查看警戒度、色情度、性欲、快感值等量化数据与明细项。
                </div>
              </div>
            </div>
          </div>
...
function parseSystemDate(
  ...
  const isoLike = /(\d{4})-(\d{1,2})-(\d{1,2})/.exec(rawDate);
  const cnLike = /(\d{1,2})\s*月\s*(\d{1,2})\s*日/.exec(rawDate);
...
const CalendarDarkApp: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const loadSystem = async () => {
    ...
    setSystem(sys);
    setCurrentDate(parseSystemDate(sys?.当前日期, sys?.星期));
  };
    ...
    () => [
      {
        ...
        content: (
          <div className="space-y-3">
            <HelpSection title="催眠能量">
              使用催眠 APP 会消耗催眠能量，不同功能消耗不同。每天自然恢复至上限的 50%。
            </HelpSection>
            <HelpSection title="MC 点">用来解锁功能，以及提高催眠能量上限。</HelpSection>
            <HelpSection title="金钱">用来购买物品，也可购买催眠能量与 MC 点。</HelpSection>
            <HelpSection title="警戒度">
              <div className="space-y-2">
                ...
                <div>每增加 5 点警戒度，会让「可疑度」每天自然增加 1 点。</div>
              </div>
            </HelpSection>
            <HelpSection title="色情度">表示角色当前被挑起的性兴奋与色情倾向程度。</HelpSection>
            <HelpSection title="可疑度">
