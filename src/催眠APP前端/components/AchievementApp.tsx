...
import { UserResources, Achievement, Quest, GeneratedChallenge } from '../types';
...
export const AchievementApp: React.FC<AchievementAppProps> = ({ userData, onUpdateUser, onBack }) => {
  const [activeTab, setActiveTab] = useState<'ACHIEVEMENTS' | 'QUESTS' | 'CHALLENGES'>('ACHIEVEMENTS');
  ...
  const refreshCurrentTab = async () => {
    try {
      if (activeTab === 'ACHIEVEMENTS') {
        ...
        // 跨天自动刷新（免费，静默；仅当天首次进入时触发一次）
        void DataService.ensureDailyRefresh().then(changed => {
  ...
  const handleClaimAchievement = async (ach: Achievement) => {
    ...
    const result = await DataService.claimAchievement(ach.id, userData.mcEnergy);
    if (result.success) {
      onUpdateUser({ ...userData, mcEnergy: result.newEnergy });
  ...
  const handleClaimQuest = async (quest: Quest) => {
    const result = await DataService.claimQuest(quest.id, userData.mcEnergy);
    if (!result.success) {
      setNotice('任务尚未完成');
      setTimeout(() => setNotice(null), 2000);
      return;
    ...
    onUpdateUser({ ...userData, mcEnergy: result.newEnergy });
    setNotice(`任务完成：+${quest.rewardMcEnergy} MC`);
    setTimeout(() => setNotice(null), 2000);
    requestRefresh();
  };
  ...
  const handleRefreshChallenges = async () => {
    ...
    setRefreshing(true);
    try {
      const result = await DataService.refreshChallenges();
      if (!result.ok) {
        setNotice(`刷新失败：${result.message || '未知原因'}`);
  ...
  const handleAcceptChallenge = async (challenge: GeneratedChallenge) => {
    const result = await DataService.acceptChallenge(challenge.id);
    if (!result.success) {
      setNotice(`接取失败：${result.message || '未知原因'}`);
      setTimeout(() => setNotice(null), 2500);
      return;
    }
    setNotice(`已接取挑战：${challenge.name}`);
    setTimeout(() => setNotice(null), 2000);
    requestRefresh();
  };

  const handleCancelChallenge = async (challenge: GeneratedChallenge) => {
    const result = await DataService.cancelChallenge(challenge.id);
    if (!result.success) {
      setNotice(`取消失败：${result.message || '未知原因'}`);
      setTimeout(() => setNotice(null), 2500);
      return;
    }
    setNotice(`已取消挑战：${challenge.name}`);
    setTimeout(() => setNotice(null), 1500);
    requestRefresh();
  };

  const handleClaimChallenge = async (challenge: GeneratedChallenge) => {
    const result = await DataService.claimChallenge(challenge.id);
    if (!result.success) {
      setNotice(result.message || '任务尚未完成');
      setTimeout(() => setNotice(null), 2000);
      return;
    }
    if (typeof result.newMoney === 'number') {
  ...
  const activeChallengeCount = challenges.filter(c => c.status === 'ACTIVE' || c.status === 'COMPLETED').length;
    ...
    <div className="h-full flex flex-col bg-slate-900 text-white animate-fade-in relative overflow-hidden">
      ...
      <div className="px-4 py-4 pt-6 flex items-center justify-between z-10 bg-slate-900/80 backdrop-blur-md border-b border-white/5">
        ...
        <div className="flex items-center gap-1.5 bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20">
          ...
          <span className="text-sm font-bold text-amber-100">{Math.floor(userData.mcEnergy)}</span>
        </div>
      </div>
          ...
          className={`flex-1 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 flex items-center justify-center gap-2
            ${
              activeTab === 'CHALLENGES'
                ? 'bg-gradient-to-r from-rose-600 to-red-600 shadow-lg text-white'
                : 'bg-white/5 text-gray-400 hover:bg-white/10'
            }`}
        >
          <Swords size={16} /> 挑战
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-4 no-scrollbar z-10">
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    {ach.isClaimed ? (
                      ...
                      >
                        <Gift size={12} />领 {ach.rewardMcEnergy} MC
                      </button>
                    ) : (
                      <div className="flex flex-col items-end">
                        <span className="text-xs font-bold text-indigo-400/50">+{ach.rewardMcEnergy} MC</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {!loading && activeTab === 'CHALLENGES' && (
          <div className="space-y-3 animate-fade-in">
            {/* 刷新栏 */}
            <div className="flex items-center justify-between gap-3 p-3 rounded-2xl border border-white/10 bg-white/5">
              <div className="text-[11px] text-white/60 leading-snug">
                <div className="flex items-center gap-1.5">
                  <Coins size={13} className="text-amber-300" />
                  <span className="text-white/70">刷新一次消耗 ¥500</span>
                </div>
                <div className="mt-0.5 text-white/40">每日首次进入自动免费刷新</div>
              </div>
              <button
                onClick={() => void handleRefreshChallenges()}
                disabled={refreshing}
                className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl border border-rose-500/30 bg-rose-500/10 text-rose-200 text-xs font-bold hover:bg-rose-500/20 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            <div className="flex items-center justify-between text-[11px] text-white/60 px-1">
              <div className="flex items-center gap-2">
                <Swords size={14} className="text-white/60" />
                <span>每日挑战</span>
              </div>
              <div className="text-white/60">
                进行中：<span className="text-white font-bold">{activeChallengeCount}</span>/3
              </div>
            </div>
            ...
            {challenges.map(c => {
              ...
              const icon =
                c.status === 'COMPLETED' ? (
                  ...
                  <Swords size={18} className="text-white/70" />
                );

              const canAccept = c.status === 'AVAILABLE' && activeChallengeCount < 3;
              const canClaim = c.status === 'COMPLETED';
              const canCancel = c.status === 'ACTIVE' || c.status === 'COMPLETED';
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-rose-200/70 mt-1 pr-4">目标：{c.target}</p>
                        <p className="text-xs text-gray-400 mt-1 pr-4 leading-snug">{c.description}</p>
                        {c.status !== 'AVAILABLE' && (
                          <div className="text-[10px] text-cyan-200/80 mt-1.5">进度：{c.progress}</div>
                        )}
                        <div className="text-[10px] text-amber-200/80 mt-2 font-bold">奖励：{c.reward}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {c.status === 'AVAILABLE' && !canAccept && (
                        <span className="text-[10px] text-white/50">已满(3)</span>
                      )}
                      {canClaim && (
                        <button
                          onClick={() => void handleClaimChallenge(c)}
                          className="bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold py-1.5 px-3 rounded-lg shadow-lg flex items-center gap-1"
                        >
                          <Gift size={12} /> 提交
                        </button>
            ...
            {challenges.length === 0 && (
              <div className="p-6 rounded-2xl border border-white/10 bg-white/5 text-center">
                <Swords size={32} className="mx-auto opacity-30 text-gray-500" />
                <p className="mt-2 text-sm text-white/60">暂无挑战任务</p>
                <p className="mt-1 text-xs text-white/40">点击上方「刷新」消耗 ¥500 生成 3 个挑战</p>
              </div>
            )}
          </div>
        )}
